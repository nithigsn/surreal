...
export default function SweatshirtDetails() {
    ...
    const { id } = useParams();
    const sweatshirt = SWEATSHIRTS[id];
    ...
    const handleClick = (index) => {
        ...
        navigate(`/sweatshirt/${index}`); // Navigate to the detail page with the tshirt index as a parameter
    };
            ...
            <div className="flex flex-col w-[90vw] h-full gap-y-3 lg:flex-row justify-center">
                ...
                <div className="lg:w-2/4 h-[370px] ">
                    ...
                    <div className="flex justify-center text-start">
                        <img src={sweatshirt.url} alt={sweatshirt.name} className="h-50 w-44" />

                    </div>
                    <div className="w-full h-55 flex justify-center gap-2">
                        <img src={sweatshirt.url} alt="" className="h-44 " />
                        <img src={sweatshirt.url} alt="" className="h-44 " />
                    </div>
                ...
                <div className="lg;w-2/4 flex gap-y-3  flex-col lg:gap-y-10">
                    ...
                    <div className="flex justify-between h-20 w-full ">
                        <div>
                            ...
                            <p>Rs. {sweatshirt.price}.00</p>
                        </div>

                        <div className="cursor-pointer">
                            {
                                favourites.find((item) => item.id === sweatshirt.id) ? <i className="fa-solid fa-heart text-red-500 hiiden lg:block" onClick={() => editFavourites(sweatshirt)}></i> : <i className="fa-regular fa-heart hiiden lg:block" onClick={() => addToFavourites(sweatshirt)}></i>
                            }
                        </div>
                    </div>
                    ...
                    <div className="details flex flex-col h-full  ">
                        <div className={`details flex flex-col border-t-[1px] border-b-[1px] border-black justify-center lg:w-[441px] ${openSections.description ? "h-40 scale-in-ver-top" : "h-10 scale-in-ver-bottom"}`}>
                            ...
                            <div className={`${openSections.description ? "block" : "hidden"}`}>
                                <p>sweatshirt in midweight sweatshirt fabric made from a cotton blend with a soft brushed inside. Jersey-lined, drawstring hood, dropped shoulders, long sleeves, a kangaroo pocket and wide ribbing at the cuffs and hem. Loose fit for a generous but not oversized silhouette.</p>
                            </div>
                        </div>
